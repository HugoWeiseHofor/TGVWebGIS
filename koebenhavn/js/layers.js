...
export function addAllLayers(map, projection, fns) {
    ...
    addGraduatedLineLayer(map, { ...styles.line_historic_coast, folder_destination: 'GeoJSON-data/Gammel_kystlinje_(1870-1899).geojson',  visible: false,               group_container: grp_fredede }, projection);
    addGraduatedLineLayer(map, { ...styles.line_piped_streams,  folder_destination: 'GeoJSON-data/Roerlagte_vandloeb.geojson',            visible: false,               group_container: grp_fredede }, projection);
    ...
    addCategorizedLayer(map,   { ...styles.catchments, folder_destination: 'GeoJSON-data/Vandoplande.geojson',     visible: false, hidden: true, group_container: grp_vandoplande }, projection);
    ...
    function updateDamageSummary() {
        if (!lyr_bygninger.getVisible()) { summaryDiv.style.display = 'none'; return; }
        ...
        let total = 0;
        jsonSource_bygninger.forEachFeatureInExtent(extent, feature => {
        ...
        summaryDiv.innerHTML =
            'Samlede gns. skadesomkostninger før tiltag [DKK/år] i nuværende udsnit:<br/>' +
            '<i>Klik på inividuel bygning for yderligere info</i><br/><strong>' +
    ...
    jsonSource_bygninger.once('change', () => {
